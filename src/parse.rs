...
impl<'a> Log<'a> {
    ...
    pub fn from(log: &str) -> Result<Log, Error> {
        // split lines and remove comment line
        let mut lines: Vec<&str> = log.lines().collect();
        if lines.len() < 2 {
            return Error::from("Die Log-Datei ist fehlerhaft");
        }
        lines.remove(0);
    ...
    */
    ...
    pub fn at_key(&self, key: &str) -> Result<Vec<f64>, Error> {
    ...
    pub fn get_xy(&self, key1: &str, key2: &str) -> Result<Vec<XY>, Error> {
...
pub fn to_xy(values1: &[f64], values2: &[f64]) -> Result<Vec<XY>, Error> {
    // initialize vector
    let mut tuples = Vec::with_capacity(values1.len());
    for &value1 in values1 {
        // add each item of first vector
